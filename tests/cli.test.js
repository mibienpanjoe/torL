...
describe('cli', () => {
  ...
  it('parses multiple inputs', () => {
    const options = parseArgs(['node', 'torl', 'a.torrent', 'b.torrent', 'magnet:?xt=urn:btih:abc']);
  ...
  it('parses --concurrency option', () => {
    const options = parseArgs(['node', 'torl', 'a.torrent', 'b.torrent', '--concurrency', '2']);
    assert.strictEqual(options.concurrency, 2);
  });

  it('parses -c shorthand', () => {
    const options = parseArgs(['node', 'torl', 'a.torrent', '-c', '3']);
    assert.strictEqual(options.concurrency, 3);
  });

  it('throws on invalid concurrency', () => {
    assert.throws(() => parseArgs(['node', 'torl', 'a.torrent', '-c', '0']), /Invalid concurrency/);
    assert.throws(() => parseArgs(['node', 'torl', 'a.torrent', '-c', 'abc']), /Invalid concurrency/);
  });

  it('throws on missing concurrency value', () => {
    assert.throws(() => parseArgs(['node', 'torl', 'a.torrent', '-c']), /requires a value/);
  });
